...
// for Gdbix and returns half of the allowance to assign to the database.
...
func MakeSystemNode(name, version string, relconf release.Config, extra []byte, ctx *cli.Context) *node.Node {
	...
	// Configure the Dubaicoin service
	...
	case ctx.GlobalBool(OlympicFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			dbixConf.NetworkId = 1
		}
		dbixConf.Genesis = core.OlympicGenesisBlock()

	case ctx.GlobalBool(TestNetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			dbixConf.NetworkId = 2
		}
		dbixConf.Genesis = core.TestNetGenesisBlock()
		state.StartingNonce = 1048576 // (2**20)

	case ctx.GlobalBool(DevModeFlag.Name):
		// Override the base network stack configs
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			stackConf.DataDir = filepath.Join(os.TempDir(), "/dubaicoin_dev_mode")
		}
		...
		if !ctx.GlobalIsSet(GasPriceFlag.Name) {
			dbixConf.GasPrice = new(big.Int)
		}
		if !ctx.GlobalIsSet(WhisperEnabledFlag.Name) {
			shhEnable = true
		}
		dbixConf.PowTest = true
	}
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return dbix.New(ctx, dbixConf)
	}); err != nil {
		Fatalf("Failed to register the Dubaicoin service: %v", err)
	}
	if shhEnable {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Gdbix release oracle service: %v", err)
	}
	return stack
}
