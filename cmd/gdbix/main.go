...
// gdbix is the official command-line client for Dubaicoin.
package main
...
import (
	...
	"github.com/dbix-project/go-dubaicoin/internal/debug"
	...
	"github.com/dbix-project/go-dubaicoin/release"
...
func init() {
	...
	app = utils.NewApp(verString, "the go-dubaicoin command line interface")
	app.Action = gdbix
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print dubaicoin version numbers",
			Description: `
...
`,
	...
	app.Before = func(ctx *cli.Context) error {
		...
		dbix.EnableBadBlockReporting = true
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if err := stack.Service(&dubaicoin); err != nil {
		utils.Fatalf("ethereum service not running: %v", err)
	}
	accman := dubaicoin.AccountManager()
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		if err := dubaicoin.StartMining(ctx.GlobalInt(utils.MinerThreadsFlag.Name), ctx.GlobalString(utils.MiningGPUFlag.Name)); err != nil {
...
func gpubench(ctx *cli.Context) error {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gdbix gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
...
func version(c *cli.Context) error {
	...
	fmt.Println("Protocol Versions:", dbix.ProtocolVersions)
